...
const laterAdjustment=JSON.parse(fs.readFileSync(new URL('docs/three-zone-location-adjustments.json',root),'utf8'));
...
test('promotion and later location work preserve the remaining 81 regions',()=>{
 const untouched=Object.entries(payload.zones).filter(([n])=>!manifest.promotedNames.includes(n)&&!laterAdjustment.changedNames.includes(n));assert.equal(untouched.length,laterAdjustment.unchangedZoneCount);assert.equal(hash(untouched),laterAdjustment.unchangedZonesSha256);assert.equal(hash(untouched.map(([n,z])=>[n,z.samplingGeometry])),laterAdjustment.unchangedGeometrySha256);
 for(const name of laterAdjustment.changedNames)assert.equal(hash(payload.zones[name]),laterAdjustment.changedZoneHashes[name],name);
...
test('detail evidence describes the accepted land version and preserves the remaining regions',()=>{
 ...
 assert.equal(hash(Object.entries(evidence.zones).filter(([n])=>!manifest.promotedNames.includes(n)&&!laterAdjustment.changedNames.includes(n))),laterAdjustment.unchangedEvidenceSha256);
